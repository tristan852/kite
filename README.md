...
[![Latest version](https://img.shields.io/github/v/release/tristan852/kite?style=for-the-badge)](https://github.com/tristan852/kite/releases/latest)
...
* **Opening Book**: Uses a precomputed complete 15-ply opening book containing all opening positions up to 15 ply for instant early-game solves.
...
The demo runs natively in *WebAssembly* and is generally slower than the Java library and the CLI tool, though still fast enough for typical use.
...
dependencies {
    implementation("io.github.tristan852:kite:1.19.2")
}
```
...
Use `./gradlew clean nativeCompile -PmarchNative=true` to enable CPU-specific optimizations for higher performance, but note that the resulting binary may not run on all machines.
